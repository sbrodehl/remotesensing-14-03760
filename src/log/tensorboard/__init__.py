...
def text_summary(state, event):
    ...
    with open(Path(state["dir"]) / 'experiment_settings.html', 'w', encoding="utf-8") as writer:
        writer.write(html_settings)
        writer.flush()
    with open(Path(state["dir"]) / 'experiment_diffs.html', 'w', encoding="utf-8") as writer:
...
def init_tensorboard_writer(state, event):
    ...
    log_dir = Path(state["dir"]) / (event.log_datestr_sort() + '_' + socket.gethostname() + '_' + state["tag"])
