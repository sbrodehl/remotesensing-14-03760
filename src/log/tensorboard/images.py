...
def plot_image(state, tag, img, *args, dataformats='CHW', **kwargs):
    ...
    if "global_step" in kwargs: del kwargs["global_step"]  # noqa: E701  pylint: disable=C0321
    state["tensorboard"].add_image(tag, img, global_step=global_step, dataformats=dataformats, **kwargs)
...
def plot_images(state, tag, imgs, *args, dataformats='NCHW', **kwargs):
    ...
    global_step = kwargs.get("global_step", state[state["log.steptype"].value])
    if "global_step" in kwargs: del kwargs["global_step"]  # noqa: E701  pylint: disable=C0321
    state["tensorboard"].add_images(tag, imgs, global_step=global_step, dataformats=dataformats, **kwargs)


def plot_image_with_boxes(state, tag, img, boxes, *args, labels=None, dataformats='NCHW', **kwargs):
    """Add image data to summary.
    ...
    global_step = kwargs.get("global_step", state[state["log.steptype"].value])
    if "global_step" in kwargs: del kwargs["global_step"]  # noqa: E701  pylint: disable=C0321
    state["tensorboard"].add_image_with_boxes(tag, img, boxes, global_step=global_step,
                                              dataformats=dataformats, labels=labels, **kwargs)
