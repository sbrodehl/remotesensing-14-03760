...
def model_summary(state, event):
    ...
    get_summary(
        _model,
        torch.zeros(tuple([state["batchsize"], len(state["SOURCE_channel_tensor_order"]), state["window_size"]] + state["transformed_px"])),
    )
    if state["exit"]:
...
def get_summary(model, x, *args, **kwargs):  # noqa: C901 too-complex pylint: disable=too-many-statements
    ...
    with option:
        LOGGER.info("\n" + df.replace(np.nan, "-").to_string())  # pylint: disable=logging-not-lazy
    ...
    total_output_size = sum(np.prod(s) for s in df["Output Shape"].to_numpy()) * num_bytes / mega_byte
