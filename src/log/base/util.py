...
def training_header(state, *args, **kwargs):  # args, kwargs are inputs of `main` event
    ...
    ) if ("device" in state and state["device"].lower().startswith("cuda") and "gpu" in state) else "cpu"
    s = [" ", "Experiment", state["tag"], "on", device, " "]
    seed_mode = (
        f"seed: {state['seed']} "
        if "seed" in state
        else rainbow("random-mode")
    )
...
def html_summary(state, event) -> Tuple[str, str]:
    html_repo_state = "<ul style='list-style: circle'>" + ("".join(f"<li style='margin:0 3em;'>{name}:{'clean' if len(diff) == 0 else '<b>diverged</b>'}:<code>{sha[:7]}</code></li>" for (name, diff, sha) in state["repository_state"])) + "</ul>"
    html_loaded_modules = "<ul style='list-style: circle'>" + ("".join(f"<li style='margin:0 3em;'>{s}</li>" for s in state["loaded_modules"])) + "</ul>"
    html_env = "<ul style='list-style: circle'>" + ("".join(f"<li style='margin:0 3em;'>{name}: <code>{ver}</code></li>" for (name, ver) in [("python", state["python"]), ("pytorch", state["pytorch"])])) + "</ul>"
    html_prepend = f"""
