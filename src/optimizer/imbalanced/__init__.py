...
def get_class_weights(state, event, batch):
    num_classes = (
        state[f"num_{state['PIPES']['TARGET'].name}_classes"] + 1
    )
    return torch.softmax(
        torch.log(
            compute_weighting(
                event,
                torch.squeeze(event.to_one_hot_vector(batch[state["PIPES"]["TARGET"].name], num_classes), -3),
                num_classes,
                to_one_hot=False
            )
        ),
...
def class_imbalance(state, event, current_loss):
    ...
    one_hot_target = torch.squeeze(event.to_one_hot_vector(
    ...
    if event.plot_every():
        event.optional.plot_scalar("Thresholds/Imbalanced", state["EMA_threshold"].mean.item())
    ...
    if "enable_skill_weighting" in state and state["enable_skill_weighting"]:
        ...
        combined_vec[:, :1, ...] -= torch.sum(combined_vec[:, 1:, ...], dim=1, keepdim=True, dtype=torch.bool).to(combined_vec.dtype)
        combined_vec[:, :1, ...] = torch.clamp(combined_vec[:, :1, ...], min=0)
    if state["cw_training"] is not None and state["combined_weight"] is None:
        state["combined_weight"] = torch.tensor([state["cw_training"]]).to(combined_vec.device)
    combined_map = torch.sum(torch.mul(combined_vec, state["combined_weight"]), dim=1)
    return current_loss * combined_map * torch.log(torch.tensor(combined_map.shape[1:]).prod())
...
def register(mf):
    mf.register_helpers({
        ...
        "EMA_init_weight": 98,
        "thresholds": [
            0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.875, 0.9, 0.9375, 0.96875,
            0.984375, 0.9921875, 0.99609375, 0.998046875, 0.9990234375, 0.99951171875,
            0.999755859375, 0.9998779296875, 0.99993896484375, 0.999969482421875,
        ],
        "cw_training": None,
        "cw_validation": None,
