...
def eval_loss(state, event, call_after_fwd_pass=True):
    ...
    lon_idx = [idx for idx, (key, enu) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "LON"][0]
    ...
    if "padding_px" in state and state["padding_px"]:
        ...
        for idx, px in enumerate(reversed(state["padding_px"])):
            ...
            narrow_mask = torch.narrow(narrow_mask, _idx, px, narrow_mask.shape[_idx] - 2 * px)
        narrow_mask += 1
    ...
    if "padding_px" in state and state["padding_px"]:
        for idx, px in enumerate(reversed(state["padding_px"])):
            ...
            current_loss = torch.narrow(current_loss, _idx, px, current_loss.shape[_idx] - 2 * px)
            targets = torch.narrow(targets, _idx, px, current_loss.shape[_idx] - 2 * px)
    with torch.no_grad():
        current_loss_var = torch.var(current_loss)
        event.optional.plot_scalar(f"Optimizer/{criterion.__class__.__name__}VAR", current_loss_var)
    ...
    if hasattr(event, "regularizer"):
        ...
        if event.plot_every():
            ...
            event.optional.plot_scalar(f"Optimizer/{criterion.__class__.__name__}", current_loss)
...
def optimize(state, event):
    ...
    if "padding_px" in state and state["padding_px"] and "mask" in state:
        state["logits"] *= torch.squeeze(state["mask"], -3).expand(state["logits"].shape)
        state["sample"][state["PIPES"]["TARGET"].name] *= state["mask"]
    lon_idx = [idx for idx, (key, enu) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "LON"][0]
    repeats = [1] * state["logits"].ndim
    repeats[1] = state["logits"].shape[1]
    lon_cond = (state["sample"][state["PIPES"]["META"].name][:, lon_idx, -1:].repeat(*repeats) < 9999.0)
...
def register_optimizer_with_defaults(event, mf_obj, Optimizer, with_closure=False, overwrite=None, init_optimizer=None):
    if not init_optimizer:
        def init_optimizer(state, net, parameters=None):
            ...
            LOGGER.info(f"Trainable parameters in model: {state['trainable_parameters']}")
...
def register(mf):
    ...
    mf.register_event('register_optimizer_with_defaults', register_optimizer_with_defaults)
