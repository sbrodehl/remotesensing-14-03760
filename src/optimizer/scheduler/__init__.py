...
def after_init_optimizer(state, event):
    ...
    last_batch_index = state["start_epoch"] * steps_per_epoch if state["start_epoch"] > 0 else -1
    epochs = state["overwrite_epochs"] if state["overwrite_epochs"] > 0 else state["epochs"]
