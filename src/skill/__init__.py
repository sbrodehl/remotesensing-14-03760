...
class Skill:
    ...
    def __call__(  # noqa: C901 too-complex pylint: disable=too-many-statements,E1136
        ...
        **kwargs,
        ...
        if search_region_radius_px is not None:
            self.search_region_px = 2 * search_region_radius_px + 1
            self.pad_pixels = search_region_radius_px
            self.conv_weights = torch.from_numpy(self.generate_disc_kernel(self.search_region_px))
        with torch.no_grad():
            _threshold = (
            ...
            weighting = (
                ...
                if weighting is not None
                    ...
                    state["mask"] if "mask" in state else None
                )
            )
            ...
            predictions = predictions.to(probabilities.dtype if locals().get("probabilities", None) is not None else logits.dtype if locals().get("logits", None) is not None else torch.float32)
            ...
            predictions_search_region = self._convolve_tensor(predictions).to(torch.bool)
            ...
            if not quiet and event.plot_every():
                event.optional.plot_imgs("Skill/Detections", torch.cat([
            ...
            if not quiet and event.plot_every():
                event.optional.plot_scalar("Skill/mCSI", csi.detach().clone().tolist()[1])
                event.optional.plot_scalar("Skill/mFAR", far.detach().clone().tolist()[1])
                event.optional.plot_scalar("Skill/mPOD", pod.detach().clone().tolist()[1])
                if not state["log.slim"]:
                    event.optional.plot_scalar("Skill/mACC", acc.detach().clone().tolist()[1])
                    event.optional.plot_scalar("Skill/undefCSI", torch.sum(~_csi_defined))
                    event.optional.plot_scalar("Skill/undefFAR", torch.sum(~_far_defined))
                    event.optional.plot_scalar("Skill/undefPOD", torch.sum(~_pod_defined))
                    event.optional.plot_scalar("Skill/TP", tp.detach().clone()[:, 1].sum().cpu().item())
                    event.optional.plot_scalar("Skill/TN", tn.detach().clone()[:, 1].sum().cpu().item())
                    event.optional.plot_scalar("Skill/FP", fp.detach().clone()[:, 1].sum().cpu().item())
                    event.optional.plot_scalar("Skill/FN", fn.detach().clone()[:, 1].sum().cpu().item())
    ...
    def _convolve_tensor(self, tensor: torch.Tensor, booleanize: bool = True):
        ...
        kernel = self.conv_weights.view(
            1, 1, self.search_region_px, self.search_region_px
        ).repeat(
            nb_channels, 1, 1, 1,
        ).to(tensor)
        tensor = F.pad(
