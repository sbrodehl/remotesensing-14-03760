...
class PreActBlock(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        if (([1] * len(stride)) != stride if isinstance(stride, list) else stride != 1) or in_planes != out_planes:
            ...
            assert len(shortcut_kernel) == len(shortcut_stride), "Kernel and Stride sizes do not match."
            if shortcut_stride != shortcut_kernel:
                if any(k < s for (k, s) in zip(shortcut_kernel, shortcut_stride)):
                    LOGGER.warning("Kernel size is (at least in one dimensions) smaller than stride. This leads to lost information.")
                shortcut_kernel = [k if k >= s else s for (k, s) in zip(shortcut_kernel, shortcut_stride)]
            self.shortcut = nn.Sequential(
                event.normalization_layer(in_planes),
                act_3,
                nn.Conv3d(
                    in_planes, out_planes, kernel_size=shortcut_kernel, stride=shortcut_stride, bias=False
                )
            )
        else:
            self.shortcut = nn.Sequential()
...
class UNet(nn.Module):
    ...
    def setup(self):
        ...
        out_channel = int(2 ** (self.source_channels - 1).bit_length() * float(state["channel_lower_factor"]))
        self.pre_block = event.optional.pre_block(
            in_planes=in_channel, out_planes=out_channel,
            altfn=lambda *l, **kw: None
        )
        if self.pre_block is not None:
            in_channel = out_channel
        strategy_base_ch = int(self.source_channels * float(state["channel_lower_factor"])) if self.pre_block is None else in_channel
        hierarchy_channels = [
            (-1 if i == 0 else 0) + strategy_base_ch + int(
                (
                    (self.channel_hierarchy_depth - strategy_base_ch) ** (1 / float(self.num_hierarchies))
                ) ** i
        ...
        for _, out_channel in zip(range(self.num_hierarchies), hierarchy_channels):
            ...
            self.hierarchies_up.append(
                event.up_block(
                    in_planes=out_channel + in_channel, out_planes=in_channel,
        ...
        self.post_block = event.optional.post_block(
            in_planes=base_ch, out_planes=self.source_channels,
            altfn=lambda *l, **kw: None
        )
    ...
    def activations(self) -> Iterable[nn.ReLU]:
        return itertools.chain(
            iter((a for a in self.pre_block.activations) if self.pre_block is not None else []),
            ...
            iter((a for a in self.post_block.activations) if self.post_block is not None else []),
        )
