...
class Load(CacheLayer):
    ...
    def __call__(self, sample: Optional[dict]):  # noqa: C901 too-complex pylint: disable=too-many-statements
        # if it's None, nothing to do
        if sample is None:
            return None

        # check if cache is available & check if we can LOAD all pipes successfully
        if self.enable_cache and not self.refresh_cache and "_cache_digests" in sample and len(sample) == 1:
            _PIPES = sample["_cache_digests"].keys()
            # TODO: check 'contains' first, only load all pipes, iff all are available
            cached = {pipe: self.get_cache(sample["_cache_digests"][pipe], suffixes=[pipe]) for pipe in _PIPES}
            for pipe in _PIPES:  # Check if any pipe is None, which results in (partially) rebuild of tensor
                if cached[pipe] is None:
                    return None
            LOGGER.debug(f"{''.join(sample['_cache_digests'][next(iter(_PIPES))].split('-')[:-1])} - cached")
        ...
        _PIPES = list(k for k in sample.keys() if not k.startswith("_"))  # PIPES excluding private members
        ...
        del sample["_channel_tags"], sample["_source_parameters"], sample["_CHANNELS"], sample["_dt_tag"], sample["_dataset_mode"],

        # load needed raw data
        raw_data = {g: {} for g in _GROUPS}
        _channels_needed = set(
            _CHANNELS[ch]
            for pipe in iter(_PIPES)
            for ch in _channel_tags[pipe]
            if "_cache_digests" in sample and not self.contains(sample["_cache_digests"][pipe], suffixes=[pipe])
        ) if self.enable_cache and not self.refresh_cache else set(_CHANNELS[ch] for pipe in iter(_PIPES) for ch in _channel_tags[pipe])
        for ch in _channels_needed:
            chan = ch.value
            if chan.loading is not None and isinstance(chan.loading, Callable):  # pylint: disable=isinstance-second-argument-not-valid-type
                raw_data[chan.group][ch.name] = chan.loading(chan, _dt_tag, mode=_mode, **_source_parameters[chan.source])

        for pipe in iter(_PIPES):
            _cached_pipe = self.get_cache(sample["_cache_digests"][pipe], suffixes=[pipe]) if self.enable_cache and not self.refresh_cache and "_cache_digests" in sample else None
        ...
        if self.enable_cache and "_cache_digests" in sample:
            ...
            for pipe in sample["_cache_digests"]:
                self.set_cache(sample["_cache_digests"][pipe], sample[pipe], suffixes=[pipe])
                _to_del.add(pipe)  # mark digest for deletion
            for dig in _to_del:
                del sample["_cache_digests"][dig]
            assert len(sample["_cache_digests"]) == 0, "Some cache digests aren't consumed."
