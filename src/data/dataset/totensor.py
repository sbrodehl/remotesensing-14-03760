        ...
        for pipe in (p for p in sample if not p.startswith('_')):
            ...
            if img is not None:
                s[pipe] = torch.from_numpy(
                    np.moveaxis(
                        np.stack([img], axis=0),  # stack 'time' dimension (not used here, but later)
                        -1, 0
                    )  # move channels first
                )
                ...
                max_dn = s[pipe].ndim if s[pipe].ndim <= len(self.dimension_names) else len(self.dimension_names)
