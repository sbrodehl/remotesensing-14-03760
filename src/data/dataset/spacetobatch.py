...
class SpaceToBatch:
    ...
    def __init__(
        ...
        **kwargs
        ...
        assert len(self.block_shape) == len(self.padding_shape), "Shape sizes do not match."
        self.dimensions = len(self.block_shape)
        if self.composite_shape is not None:
            assert len(self.composite_shape) == len(self.block_shape), "Shape sizes do not match."
            self.composite_blocks = SDIV(np.array(self.composite_shape), self.block_shape)
            self.padded_composite_shape = self.composite_blocks * self.block_shape + 2 * self.padding_shape
            self.composite_padding = [
                i
                for sl in [
                    ((n - a) // 2, (n - a) - ((n - a) // 2))
                    for a, n in zip(reversed(self.composite_shape), reversed(self.padded_composite_shape))
                ]
                for i in sl
            ]
    ...
    def __call__(self, sample: dict):
        ...
        for pipe in (p for p in sample if not p.startswith('_')):
            if sample[pipe].nelement() > 0:
                ...
                sample[pipe] = F.pad(sample[pipe], pad=pad, mode=self.mode, value=self.value[pipe])
                ...
                sample[pipe] = sample[pipe].reshape(list(
                ...
                sample[pipe] = torch.unbind(
                    torch.movedim(sample[pipe], 2, 0).rename("STB", *_names),
                    0
                )
