...
class AbstractBINARYDataset:  # pylint: disable=invalid-name,too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-statements
        ...
        **kwargs,
        ...
        self.dataset_split: DatasetSplit = kwargs.get("dataset_split", DatasetSplit.TRAIN)
        self.window_size = window_size
        # check CHANNELS
        assert self._state["CHANNELS"] is not None and isinstance(self._state["CHANNELS"], dict)
        # check TAGS
        ...
        if self.enable_cache:
            try:  # only caching of space_transforms is support
                ...
                cache_transform_idx = next(iter(cache_transform_idx_s[::-1]))  # get latest CacheLayer
                cache_transforms = self.transforms_space.transforms[cache_transform_idx:]
                ...
                LOGGER.warning("No CacheLayer found in transforms found! Transform-cache disabled!")
        ...
        LOGGER.info(f"{self.__class__.__name__}:: Set {len(self.samples)} samples to work with.")
        ...
        self.channel_parameters = {
            p.__name__: [a for a in self.__dir__() if a.startswith(p.__name__.lower() + "_")] for p in self._state["sources.GROUPS"].values()
        }
        self.source_parameters = {
            s: [
                k for k in self._state.all.keys() if "data.sources." in k and k.split("data.sources.")[-1].split(".")[0] not in set(set(self._state["SOURCES"]) - {s})
            ] for s in self._state["SOURCES"]
        }

    def get_hash(self):
        pipe_digs = self.generate_pipe_hashes()
        return hashlib.sha512('-'.join([str(self.dataset_split), str(self.samples)] + [pipe_digs[pipe] for pipe in sorted(pipe_digs)]).encode("utf-8")).hexdigest()

    def generate_pipe_hashes(self, dt_tag=None):
        cache_digests = {}
        _gen_str = ";".join(str(e) for e in sorted(self.cache_prefixes["GENERAL"].items())) + str(dt_tag)
        for p in self._state["PIPES"].values():
            _sp = _gen_str + ";".join(str(e) for e in sorted(self.cache_prefixes[p.name].items()))
            _hexdig = hashlib.sha512(_sp.encode("utf-8")).hexdigest()
            # prepend hex digest with datetime tag
            cache_digests[p.name] = f"{str(dt_tag).replace(':', '-').replace(' ', '-')}-{_hexdig}"
    ...
    def _get_space_tensor(self, dt_tag: datetime, idx: int = None):
        ...
        if self.enable_cache and not self.refresh_cache:
            ...
            for t in self.cache_space_transforms.transforms:
                tensor = t(tensor)
                LOGGER.debug(f"Worker={worker_id} id={str(idx)} {t.__class__.__name__} shape={self._nested_tensor_shape(tensor)}.")
            ...
            if tensor is not None:
                LOGGER.debug(f"Worker={worker_id} id={str(idx)} fp={str(dt_tag)} (cached) shape={self._nested_tensor_shape(tensor)}.")
                return tensor
        ...
        tensor["_source_parameters"] = {src: {k.replace("src.data.sources.", ''): self._state[k] for k in self.source_parameters[src]} for src in self.source_parameters}
        tensor["_channel_tags"] = {p.name: self.__getattribute__(f"{p.name}_channel_tags") for p in self._state["PIPES"].values()}
    ...
    def getitem(self, idx):
        ...
        worker_id = torch.utils.data.get_worker_info().id if torch.utils.data.get_worker_info() is not None else 0
        LOGGER.debug(f"Worker={worker_id} id={idx} __getitem__ data=[{', '.join([str(dt_tag) for dt_tag in self.samples[idx]])}].")

        tensor = [self._get_space_tensor(dt_tag, idx) for dt_tag in self.samples[idx]]
        LOGGER.debug(f"Worker={worker_id} id={idx} [get_space_tensor()'s] shape={self._nested_tensor_shape(tensor)}.")
        ...
        if self.transforms_space_time is not None:
            ...
            for t in self.transforms_space_time.transforms:
                tensor = t(tensor)
                LOGGER.debug(f"Worker={worker_id} id={idx} {t.__class__.__name__} shape={self._nested_tensor_shape(tensor)}.")
        if self.measure_time:
            LOGGER.info(
    ...
    def _nested_tensor_shape(self, tensor) -> str:  # pylint: disable=too-many-return-statements
    ...
    def generate_samples(self) -> List[List[datetime.datetime]]:
        ...
        samples = {
            source: frozenset(
                ...
                if isinstance(self._state[f"{source}.samples"], dict) and self.dataset_split.value in self._state[f"{source}.samples"]
                else self._state[f"{source}.samples"]
            ) if self._state[f"{source}.samples"] is not None else None
            for source in self._state["sources.SOURCES"]
        }
        LOGGER.info(f"{self.__class__.__name__}:: Found {str({k: len(v) for k, v in samples.items() if v is not None})}.")
        samples = frozenset.intersection(*filter(lambda i: i is not None, samples.values()))
        LOGGER.info(f"{self.__class__.__name__}:: Found {len(samples)} overlapping samples.")
        if self.window_size > 1:
            samples = sorted(samples)
            # get greatest time delta of all sources
            sample_time_delta = next(iter(sorted(set(self._state[f"{source}.time_delta"] for source in self._state["sources.SOURCES"] if f"{source}.time_delta" in self._state), reverse=True)))
            # split sample keys in continuous patches
            samples = generate_continuous_patches(samples, sample_time_delta)
            LOGGER.info(f"{self.__class__.__name__}:: Found {len(samples)} samples after patch generation (delta={sample_time_delta}).")
            # apply window transformation
            samples = apply_window(samples, window_size=self.window_size)
        elif self.window_size == 1:
            samples = [[s] for s in samples]

        LOGGER.info(f"{self.__class__.__name__}:: Found {len(samples)} samples after window generation.")
