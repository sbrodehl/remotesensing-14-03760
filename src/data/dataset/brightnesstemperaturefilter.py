...
class BrightnessTemperatureFilter:
    ...
    def __call__(self, sample):
        ...
        cond = np.where((sample["AUXILIARY"]["BT"]["BT_062"] - sample["AUXILIARY"]["BT"]["BT_073"]) > -1, False, True)
        for pipe in sample:
            if "LINET" in sample[pipe] and sample[pipe]["LINET"] is not None and len(sample[pipe]["LINET"]):
                for ch in sample[pipe]["LINET"]:
                    if sample[pipe]["LINET"][ch] is not None and len(sample[pipe]["LINET"][ch]):  # noqa: E501 line-too-long
