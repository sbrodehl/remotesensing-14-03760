...
def get_data(  # noqa: C901 too-complex
    ...
    bt_filter_condition = (
        "enable_bt_filter" in state and state["enable_bt_filter"]
    ...
    if bt_filter_condition:
        ...
        assert "BT_062" in state["AUXILIARY_channel_tags"] and "BT_073" in state["AUXILIARY_channel_tags"]
        # somewhere needs to be a 'LINET' channel to filter
        if not any(
            "LINET" in state[f"{p.name}_channel_tags"]
            for p in state["PIPES"].values()
        ):
            LOGGER.warning("No 'LINET' channel found, which is needed for brightness temperature filtering.")
    ...
    density_mpd = 2 * torch.tensor([20037508.3428, 10018754.1714]) / torch.tensor([360., 180.])  # meter per degree
    mean_density_kmpp = (state["deg_per_px"] * density_mpd).mean() * 1000 ** -1   # mean kilometer per pixel
    # update true kilometers
    state["image_density_km"] = mean_density_kmpp
    state["search_region_radius_km"] = (state["search_region_radius_px"] + 0.5) * mean_density_kmpp
    LOGGER.info("Computed Search Region (SR):"
    ...
    if "zero_out" in state and len([s.strip() for s in state["zero_out"] if s.strip()]) > 0:
        space_transforms += [ZeroOutTensors([s.strip().split("::") for s in state["zero_out"] if s.strip()])]
    ...
    with_transform_cond = (
        ...
        if with_transform is not None
            ...
            state["with_transform"]
            if "with_transform" in state
            else False
        )
    )
    if with_transform_cond:
        transform += [t for tr in event.optional.dataset_transform(ds_split) for t in tr]
        if "sample_reduction" in state and state["sample_reduction"]:
            space_time_transforms += [state["sample_reduction"].value(
            ...
            try:
                ...
                state["padded_composite_shape"] = space_time_transforms[-1].padded_composite_shape
    ...
    if ("enable_filter_empty" in state and state["enable_filter_empty"]
            and "filter_empty" in state and state["filter_empty"]
            and ds_split == DatasetSplit.TRAIN):
    ...
    for p in state["PIPES"].values():
        cache_prefixes.__setitem__(p.name, {"CHANNELS": [c[0] for c in sorted(state[f"{p.name}_channel_tensor_order"])]})
        for ch in [c[0] for c in sorted(state[f"{p.name}_channel_tensor_order"])]:
            if ch == "LINET":
                cache_prefixes[p.name]["LINET"] = list(map(str, [
                    state["LINET.min_amp"], state["LINET.lead_time"], state["LINET.time_range"],
                ]))

    shffl_str = "enable_shuffle_samples"
    shuffle_samples = state[shffl_str] if shffl_str in state and state[shffl_str] and ds_split == DatasetSplit.TRAIN else False
