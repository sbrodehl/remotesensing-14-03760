...
class IterableBINARYDataset(
    ...
    def __iter__(self):
        ...
        LOGGER.info(f"Worker={worker_id_str} {split_size}/{len(dataset.samples)} samples (range='[{start},{end})').")
        ...
        for idx in indices:
            ...
            if isinstance(ret, list):
                LOGGER.debug(f"Worker={worker_id_str} id={idx} yields {len([s for s in ret if s is not None])} samples.")
                for s in ret:
                    if s is not None:
                        seen_samples += 1
                        yield s
