...
def sample_iter(start: datetime.datetime, end: datetime.datetime, delta: datetime.timedelta):
    while start <= end:
        yield start
        start += delta
...
def setup(state):
    ...
    state["cnx"] = partial(sqlite3.connect, f"file:{str(state['db_path'])}?mode=ro", uri=True, detect_types=sqlite3.PARSE_DECLTYPES | sqlite3.PARSE_COLNAMES)
    cnx, asc, desc = None, None, None
    try:  # connect to database
        cnx = state["cnx"]()
        c = cnx.cursor()
        c.execute("""SELECT "Event-Time" from LINET ORDER BY "Event-Time" ASC LIMIT 1;""")
        asc: datetime.datetime = c.fetchone()[0]
        asc = asc.astimezone(timezone.utc).replace(second=0, microsecond=0)
        c.execute("""SELECT "Event-Time" from LINET ORDER BY "Event-Time" DESC LIMIT 1;""")
        desc: datetime.datetime = c.fetchone()[0] - datetime.timedelta(minutes=state["time_range"])
    ...
    state.all[f"{state['module_id']}.samples"] = list(sample_iter(asc, desc, state["time_delta"]))
    # register
    state["SOURCES"].append(state["module_name"])


def query_db(sql_query: str, cnx, from_timestamp: datetime.datetime, to_timestamp: datetime.datetime):
...
def get_data(channel, dt, **kwargs):
    ...
    events = query_db(kwargs["LINET.sql_query"], kwargs["LINET.cnx"], start_date, end_date)
    ...
    events['EventTime'] = pd.to_datetime(events['EventTime'], utc=True)  # convert to timestamp
