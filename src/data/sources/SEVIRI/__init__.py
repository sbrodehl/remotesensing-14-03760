...
def inspect_data(root: Path, suffix: str, *args, meta_suffix=".meta", **kwargs):
    ...
    packages = [os.path.join(_root, file) for _root, dirs, files in os.walk(root) for file in files if file.endswith(suffix)]
    all_frames = []
    for pkg in packages:
        frames = []
        ...
        with tarfile.open(pkg) as tar:
            for member in tar.getmembers():
                with tarfile.open(fileobj=tar.extractfile(member)) as frame_tar:
                    ...
                    assert readme_obj.name == "README.txt", "First member in tar-file must be README.txt!"
                    readme = codecs.getreader("utf-8")(frame_tar.extractfile(readme_obj)).read().split('\n')
                    pro_file = next(iter([ff for ff in readme if '-_________-PRO______-' in ff]))
                    dt = pro_file.split('-')[-2]
        ...
        all_frames.extend(frames)
    LOGGER.info(f"Inspecting data at location '{root}' - found {len(all_frames)} frames in {len(packages)} packages.")
    return all_frames
...
def group_samples_by(samples, fn, split_order):
    ...
    LOGGER.warning(f"Grouped {sum(len(g[1]) for g in grouped_dt)} / {len(merged_dt)} samples.")
    assert sum(len(g[1]) for g in grouped_dt) == len(merged_dt), "Some samples are lost."
    if len(grouped_dt) < len(split_order):
        LOGGER.warning(f"Some Splits may not have samples! Found {len(grouped_dt)} but got {len(split_order)} splits in order.")
...
def setup(state):
    ...
    roots = {
        split.value: sorted([Path(r).expanduser().absolute().resolve() for r in state[f'root.{split.value}'] if r and len(r) > 0])
    ...
    split_order = [DatasetSplit.TRAIN, DatasetSplit.TEST, DatasetSplit.TRAIN, DatasetSplit.VALIDATION]
    ...
    if state["enable_pkg_cache"]:
        packages = {
            mode: {
                os.path.join(_root, file): {} for r in roots[mode] for _root, dirs, files in os.walk(r) for file in files if file.endswith(state["suffix"])
            } for mode in roots
        }
        seviri_tags = set(ch for pipe in iter(PIPES) for ch in state[f"{pipe.name}_channel_tags"] if ch in set(map(lambda e: getattr(e, "name"), set(HRIT) | set(BT))))
        for mode in roots:
            for r in roots[mode]:
                for obj in inspect_data(r, state["suffix"]):
    ...
    try:
        dt = next(chain.from_iterable(state.all[f"{state['module_id']}.samples"][split.value] for split in DatasetSplit))
        get_data(HRIT.VIS006.value, dt, **{
            "mode": next(filterfalse(lambda item: item is None, [split if dt in state.all[f"{state['module_id']}.samples"][split.value] else None for split in DatasetSplit])),
...
def get_data(channel, dt, **kwargs):
    ...
    pkg, frame = kwargs["SEVIRI.samples"][mode.value if mode is not None else "train"][dt]
    ...
    if enable_pkg_cache and pkg_cache is not None and pkg in pkg_cache[mode.value]:
        cache_pkg_pt = Path(kwargs["SEVIRI.resample_cache_dir"]) / f"{hashlib.sha512(pkg.encode('utf-8')).hexdigest()}-{tar_pkg.name}"
        ...
        with tarfile.open(fileobj=tar.extractfile(frame)) as frame_tar:
            ...
            fs_files = [
                MemoryFile(
                    ...
                    xRIT_decompress(frame_tar, f) for f in frame_tar if f.name != "README.txt" and f.name.split('-')[4] in {f"{channel.tag}___", '_________'}
                )]
            ...
            assert len(scn.missing_datasets) == 0 and len(scn.wishlist) != 0, "Not all SEVIRI channels available for this frame."
            scn = scn.resample(kwargs["area_config"], cache_dir=kwargs["SEVIRI.resample_cache_dir"])
...
def register(mf):
    ...
    mf.register_helpers({
        ...
        "suffix": ".tar",
        "calibration_counts": [0.6, 0.8, 1.6, 3.9, 6.2, 7.3, 8.7, 9.7, 10.8, 12.0, 13.4],
