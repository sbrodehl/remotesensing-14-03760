...
def setup(state):
    ...
    state.all[f"{state['module_id']}.samples"] = None  # None == every time step is available
...
class SPACE(Enum):
    LAT = Channel("LAT", "SPACE", "SPACE", loading=get_data, normalization=lambda tensor: tensor / np.float32(90.))
    LON = Channel("LON", "SPACE", "SPACE", loading=get_data, normalization=lambda tensor: tensor / np.float32(180.))
