...
class Channel:  # pylint: disable=too-few-public-methods
    ...
    def __init__(
        ...
        self.name = name
        if operator is None or isinstance(operator, Callable):  # pylint: disable=isinstance-second-argument-not-valid-type
            self.operator = operator
        else:
            raise RuntimeError(f"operator type ({type(operator)}) not supported.")
        if normalization is None or isinstance(normalization, Callable):  # pylint: disable=isinstance-second-argument-not-valid-type
            self.normalization = normalization
        else:
            raise RuntimeError(f"normalization type ({type(normalization)}) not supported.")
        if loading is None or isinstance(loading, Callable):  # pylint: disable=isinstance-second-argument-not-valid-type
...
def post_setup(state):
    ...
    _members = [(grp.__name__, grp) for source in state['SOURCES'] for grp in state[f"{source}.GROUPS"]]
    GROUPS = state["GROUPS"] = OrderedDict(sorted({n: v for n, v in _members}.items()))  # pylint: disable=unnecessary-comprehension
    channel_dict = OrderedDict({ch.name: ch for _, v in state["GROUPS"].items() for ch in v})
