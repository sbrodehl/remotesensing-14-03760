...
def dump_sample(state, comps, z, name):
    for (lat, lon, year, month, day, hour, minute, targets, in_ch, pers_pred, pers_skill, m_logits, m_pred, m_skill) in z:
        ...
        dt_c.append({
            ...
            "MODEL_LOGITS": m_logits.to(torch.float32) if m_logits is not None else torch.zeros_like(pers_pred, dtype=torch.float32) * float("nan"),
            "MODEL_PREDICTIONS": m_pred.to(torch.float32) if m_pred is not None else torch.zeros_like(pers_pred, dtype=torch.float32) * float("nan"),
            "MODEL_SKILL": m_skill.to(torch.int8) if m_skill is not None else torch.zeros_like(pers_skill, dtype=torch.float32) * float("nan"),
        })
        comps[tag] = dt_c
...
def dump_details_json(state):
    ...
    with open(Path(state["log.dir"]) / "validation_persistence_details.json", "w", encoding="utf-8") as fh:
    ...
    if "enable_testing" in state and state["enable_testing"]:
        ...
        with open(Path(state["log.dir"]) / "testing_persistence_details.json", "w", encoding="utf-8") as fh:
            json.dump(state["persistence_testing_details"], fh)
    if "model" in state and "network_pt" in state and state["network_pt"] and len([pt for pt in state["network_pt"] if len(pt) > 0]) > 0 and len(state['model_csi']) > 0:
        compute_details_stats(state, "model_details")
        with open(Path(state["log.dir"]) / "validation_model_details.json", "w", encoding="utf-8") as fh:
            json.dump(state["model_details"], fh)
        if "enable_testing" in state and state["enable_testing"]:
            compute_details_stats(state, "model_testing_details")
            with open(Path(state["log.dir"]) / "testing_model_details.json", "w", encoding="utf-8") as fh:
                json.dump(state["model_testing_details"], fh)


def accumulate_models(state):
    if "model" in state and "network_pt" in state and state["network_pt"] and len([pt for pt in state["network_pt"] if len(pt) > 0]) > 0 and len(state['model_csi']) > 0:
        LOGGER.info(f"Avg. Batch CSI Validation (Multi-Model): {np.nanmean(state['m_model_csi'], axis=0)[1]:.2f} +/- {np.nanstd(state['m_model_csi'], axis=0)[1]:.2f}")
        LOGGER.info(f"Complete CSI Validation (Multi-Model): {np.nanmean(state['model_csi']):.2f} +/- {np.nanstd(state['model_csi']):.2f}")
        if "enable_testing" in state and state["enable_testing"]:
            LOGGER.info(f"Avg. Batch CSI Testing (Multi-Model): {np.nanmean(state['testing_m_model_csi'], axis=0)[1]:.2f} +/- {np.nanstd(state['testing_m_model_csi'], axis=0)[1]:.2f}")
            LOGGER.info(f"Complete CSI Testing (Multi-Model): {np.nanmean(state['testing_model_csi']):.2f} +/- {np.nanstd(state['testing_model_csi']):.2f}")
...
def main(state, event):  # noqa: C901 too-complex pylint: disable=too-many-statements
    ...
    with state.temporary({
        ...
        if not ds_hash_fp.exists():
            ...
            with torch.no_grad():
                tqdm_batch = tqdm(desc="Caching Validation Batches", **state["tqdm_kwargs"])
        ...
        if "enable_testing" in state and state["enable_testing"]:
            ...
            if not ds_hash_fp.exists():
                ...
                with torch.no_grad():
                    ...
                    tqdm_batch.close()
                ds_hash_fp.touch()
    state["persistence_csi"], state["m_per_csi"], state["m_per_far"], state["m_per_pod"] = [], [], [], []
    state["persistence_details"], state["persistence_testing_details"] = {}, {}
    state["testing_persistence_csi"], state["testing_m_per_csi"], state["testing_m_per_far"], state["testing_m_per_pod"] = [], [], [], []
    ...
    if "network_pt" in state and state["network_pt"] and len(state["network_pt"]) > 0:
        ...
        state["model_csi"], state["m_model_csi"], state["m_model_far"], state["m_model_pod"] = [], [], [], []
        state["testing_model_csi"], state["testing_m_model_csi"], state["testing_m_model_far"], state["testing_m_model_pod"] = [], [], [], []
        pt_idx = len(state["network_pt"])
        for pt in state["network_pt"]:
            pt_idx -= 1
            state["network_pt_hash"] = hashlib.sha512(str(pt).encode("utf-8")).hexdigest()
            ...
            assert f"{state['model'].__class__.__name__}" in checkpoint, "Model class not found in checkpoint."
            LOGGER.info(f"Loading '{state['model'].__class__.__name__}': {pt}")
            state["model"].load_state_dict(checkpoint[f"{state['model'].__class__.__name__}"])
...
def compute_skill(false_detected: Tensor, correct_detected: Tensor, miss_detected: Tensor, nan_mask: Tensor):
    ...
    """
    tp = torch.nansum(nan_mask.expand(correct_detected.shape) * correct_detected, dim=tuple(range(2, len(correct_detected.shape))))
    fn = torch.nansum(nan_mask.expand(miss_detected.shape) * miss_detected, dim=tuple(range(2, len(miss_detected.shape))))
    fp = torch.nansum(nan_mask.expand(false_detected.shape) * false_detected, dim=tuple(range(2, len(false_detected.shape))))
...
def compute_details_stats(state, detail_tag: str):
    for h in state[detail_tag]:
        for m in state[detail_tag][h]:
            state[detail_tag][h][m]["mcsi"] = np.array(state[detail_tag][h][m]["csi"]).mean(axis=(0, 1)).tolist()
            state[detail_tag][h][m]["mfar"] = np.array(state[detail_tag][h][m]["far"]).mean(axis=(0, 1)).tolist()
            state[detail_tag][h][m]["mpod"] = np.array(state[detail_tag][h][m]["pod"]).mean(axis=(0, 1)).tolist()
            state[detail_tag][h][m]["csis"] = np.array(state[detail_tag][h][m]["csi"]).tolist()
            state[detail_tag][h][m]["fars"] = np.array(state[detail_tag][h][m]["far"]).tolist()
            state[detail_tag][h][m]["pods"] = np.array(state[detail_tag][h][m]["pod"]).tolist()
            state[detail_tag][h][m]["csi"] = [None for _ in range(len(state[detail_tag][h][m]["tp"]))]
            state[detail_tag][h][m]["far"] = [None for _ in range(len(state[detail_tag][h][m]["tp"]))]
            state[detail_tag][h][m]["pod"] = [None for _ in range(len(state[detail_tag][h][m]["tp"]))]
            for n_idx in range(len(state[detail_tag][h][m]["tp"])):
                state[detail_tag][h][m]["csi"][n_idx] = (100. * np.divide(
                    np.sum(state[detail_tag][h][m]["tp"][n_idx]),
                    (np.sum(state[detail_tag][h][m]["tp"][n_idx]) + np.sum(state[detail_tag][h][m]["fn"][n_idx]) + np.sum(state[detail_tag][h][m]["fp"][n_idx]))
                )).tolist()
                state[detail_tag][h][m]["far"][n_idx] = (100. * np.divide(
                    np.sum(state[detail_tag][h][m]["fp"][n_idx]),
                    (np.sum(state[detail_tag][h][m]["tp"][n_idx]) + np.sum(state[detail_tag][h][m]["fp"][n_idx]))
                )).tolist()
                state[detail_tag][h][m]["pod"][n_idx] = (100. * np.divide(
                    np.sum(state[detail_tag][h][m]["tp"][n_idx]),
                    (np.sum(state[detail_tag][h][m]["tp"][n_idx]) + np.sum(state[detail_tag][h][m]["fn"][n_idx]))
                )).tolist()
...
def stitch_composite(state, tag, composite, name="composite-results"):
    ...
    if "composite_blocks" in state and np.array(state["composite_blocks"]).prod() == 1 == len(composite):
        return False
    idx_lut: dict = {tag: idx for idx, tag in enumerate(sorted(next(iter(composite)).keys()))}
    composite = [(
    ...
    composite = [composite[s:e] for (s, e) in [(ridx * len(composite) // blocks[0], len(composite) // blocks[0] + ridx * len(composite) // blocks[0]) for ridx in range(blocks[0])]]
    for ridx, row in enumerate(composite):
        row.sort(key=lambda largs: largs[1])
        composite[ridx] = [i[-1] for i in row]
    composite = {tt: torch.stack([torch.stack([e[tt] for e in row], dim=0) for row in composite], dim=0) for tt in idx_lut.keys()}
    for tt in composite:
        t = composite[tt]
        ...
        t = t.reshape(*list(t.shape[:-4]), t.shape[-4] * t.shape[-3], t.shape[-2] * t.shape[-1])
        if "composite_padding" in state and state["composite_padding"]:
            for idx, (px, block_px) in enumerate(zip(np.array(state["composite_padding"]).reshape((2, -1)).tolist(), reversed(state["padding_px"]))):
                _idx = len(t.shape) - (idx + 1)
                t = torch.narrow(t, _idx, px[0] - block_px, t.shape[_idx] - (sum(px) - 2 * block_px))
        composite[tt] = t.detach().cpu().numpy()
    dd = Path(state['log.dir']) / name / f"epoch-{state['current_epoch'] if 'current_epoch' in state else 'final'}"
...
def validate(state, event):  # noqa: C901 too-complex pylint: disable=too-many-statements
    ...
    with torch.no_grad():
        for sample in state["validation_dataloader"]:
            sample = {k: event.optional.to_device(v, altfn=lambda a: a) for k, v in sample.items()}
            if MODEL_AVAILABLE:
                out = state["model"](sample[state["PIPES"]["SOURCE"].name])
                out = out.detach().clone().cpu()
            dummy_out = torch.squeeze(sample[state["PIPES"]["TARGET"].name], 1).detach().clone().cpu()
            lat_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "LAT"][0]
            lon_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "LON"][0]
            year_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "YEAR"][0]
            month_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "MONTH"][0]
            day_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "DAY"][0]
            hour_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "HOUR"][0]
            minute_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["META"].name}_channel_tensor_order']) if key == "MINUTE"][0]
            hour = int(sample["META"][:, hour_idx, -1][sample["META"][:, hour_idx, -1] != 9999][0].item())
            minute = int(sample["META"][:, minute_idx, -1][sample["META"][:, minute_idx, -1] != 9999][0].item())
            repeats = [1] * dummy_out.ndim
            repeats[1] = dummy_out.shape[1]
            mask = (sample[state["PIPES"]["META"].name][:, lon_idx, -1:].repeat(*repeats) < 9999.0)[:, -1:].unsqueeze(1).cpu().clone()
            if "padding_px" in state and state["padding_px"]:
                mask |= torch.zeros_like(dummy_out, dtype=torch.int8)[:, -1:].unsqueeze(1).to(torch.bool).clone()
            ...
            if MODEL_AVAILABLE:
                ...
                model_predictions = (probabilities >= model_threshold).to(probabilities.dtype)
                nn_skill = torch.cat([
                ...
                (mcsi, mfar, mpod), (stp, sfn, sfp) = compute_skill(state["fd"], state["cd"], state["md"], nan_mask)
                ...
                if minute not in state["model_details"][hour]:
                    state["model_details"][hour][minute] = copy.deepcopy(DETAILS_TEMPLATE)
            ...
            persistence_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["SOURCE"].name}_channel_tensor_order']) if key == "PERSISTENCE"][0]
            persistence_model = sample[state["PIPES"]["SOURCE"].name][:, persistence_idx, -1:].detach().to(dummy_out.device)
            ...
            event.Skill(
                ...
                targets=sample[state["PIPES"]["TARGET"].name].detach().to(dummy_out.device),
                quiet=True,
                search_region_radius_px=state["search_region_radius_px"],
                reduction=None,
                reduce_non_nan=True,
            ...
            if minute not in state["persistence_details"][hour]:
                state["persistence_details"][hour][minute] = copy.deepcopy(DETAILS_TEMPLATE)
            ...
            state["persistence_details"][hour][minute]["linets"][-1].append(lights.item())
            if state["dump_results"]:
                dump_sample(state, comps, zip(
                    ...
                    targets,
                    sample[state["PIPES"]["SOURCE"].name][:, :, -1].detach().cpu().clone(),
                    sample[state["PIPES"]["SOURCE"].name][:, persistence_idx:persistence_idx + 1, -1:].detach().cpu().clone(),
                    persistence_skill,
                    out.detach().cpu().clone() if MODEL_AVAILABLE else [None] * state["batchsize"],
                    model_predictions.detach().cpu().clone() if MODEL_AVAILABLE else [None] * state["batchsize"],
                    nn_skill.detach().cpu().clone() if MODEL_AVAILABLE else [None] * state["batchsize"],
                ), "validation-results")
            tqdm_batch.update()
    tqdm_batch.close()
    if MODEL_AVAILABLE:
        m_model_csi = np.array(m_model_csi) * 100.
        m_model_far = np.array(m_model_far) * 100.
        m_model_pod = np.array(m_model_pod) * 100.
    m_per_csi = np.array(m_per_csi) * 100.
    m_per_far = np.array(m_per_far) * 100.
    m_per_pod = np.array(m_per_pod) * 100.
    with np.errstate(divide='ignore', invalid='ignore'):
        persistence_csi = 100. * np.divide(persistence_tp, (persistence_tp + persistence_fn + persistence_fp))
        if MODEL_AVAILABLE:
            model_csi = 100. * np.divide(model_tp, (model_tp + model_fn + model_fp))
    if not np.isnan(persistence_csi):
        LOGGER.info(f"Avg. Batch CSI Validation (Persistence): {np.nanmean(m_per_csi, axis=0)[1]:.2f} +/- {np.nanstd(m_per_csi, axis=0)[1]:.2f}")
        LOGGER.info(f"Complete CSI Validation (Persistence): {persistence_csi:.2f}")
    if MODEL_AVAILABLE and not np.isnan(model_csi):
        LOGGER.info(f"Avg. Batch CSI Validation (Model): {np.nanmean(m_model_csi, axis=0)[1]:.2f} +/- {np.nanstd(m_model_csi, axis=0)[1]:.2f}")
        LOGGER.info(f"Complete CSI Validation (Model): {model_csi:.2f}")
        event.optional.plot_scalar("Model/Validation/CSI", model_csi)
        event.optional.plot_scalar("Model/Validation/mCSI", np.nanmean(m_model_csi, axis=0)[1])
        event.optional.plot_scalar("Model/Validation/mFAR", np.nanmean(m_model_far, axis=0)[1])
        event.optional.plot_scalar("Model/Validation/mPOD", np.nanmean(m_model_pod, axis=0)[1])
    ...
    if not np.isnan(persistence_csi):
        ...
        event.optional.plot_scalar("Persistence/Validation/mCSI", np.nanmean(m_per_csi, axis=0)[1])
        event.optional.plot_scalar("Persistence/Validation/mFAR", np.nanmean(m_per_far, axis=0)[1])
        event.optional.plot_scalar("Persistence/Validation/mPOD", np.nanmean(m_per_pod, axis=0)[1])
...
def testing(state, event):  # noqa: C901 too-complex pylint: disable=too-many-statements
    ...
    with torch.no_grad():
        for sample in state["testing_dataloader"]:
            ...
            lights = torch.nansum(targets)
            if MODEL_AVAILABLE:
                model_threshold = state["Model_Threshold"] if not state["use_time_thresholds"] else state["time_thresholds"][(hour, minute)][0]
                event.Skill(
                    state,
                    event,
                    logits=out,
                ...
                if minute not in state["model_testing_details"][hour]:
                    state["model_testing_details"][hour][minute] = copy.deepcopy(DETAILS_TEMPLATE)
                ...
                state["model_testing_details"][hour][minute]["linets"][-1].append(lights.item())
            persistence_idx = [idx for idx, (key, _) in enumerate(state[f'{state["PIPES"]["SOURCE"].name}_channel_tensor_order']) if key == "PERSISTENCE"][0]
            persistence_model = sample[state["PIPES"]["SOURCE"].name][:, persistence_idx, -1:].detach().to(dummy_out.device)
            persistence_model = torch.cat([
                torch.ones_like(persistence_model) - persistence_model,
            ...
            if minute not in state["persistence_testing_details"][hour]:
                state["persistence_testing_details"][hour][minute] = copy.deepcopy(DETAILS_TEMPLATE)
            ...
            state["persistence_testing_details"][hour][minute]["linets"][-1].append(lights.item())
            if state["dump_results"]:
                dump_sample(state, comps, zip(
                    sample["META"][:, lat_idx, -1].detach().cpu().clone(),
                    sample["META"][:, lon_idx, -1].detach().cpu().clone(),
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        if MODEL_AVAILABLE:
            model_csi = 100. * np.divide(model_tp, (model_tp + model_fn + model_fp))
    LOGGER.info(f"Avg. Batch CSI Testing (Persistence): {np.nanmean(m_per_csi, axis=0)[1]:.2f} +/- {np.nanstd(m_per_csi, axis=0)[1]:.2f}")
    LOGGER.info(f"Complete CSI Testing (Persistence): {persistence_csi:.2f}")
    if MODEL_AVAILABLE:
        LOGGER.info(f"Avg. Batch CSI Testing (Model): {np.nanmean(m_model_csi, axis=0)[1]:.2f} +/- {np.nanstd(m_model_csi, axis=0)[1]:.2f}")
        LOGGER.info(f"Complete CSI Testing (Model): {model_csi:.2f}")
        event.optional.plot_scalar("Model/Testing/CSI", model_csi)
        event.optional.plot_scalar("Model/Testing/mCSI", np.nanmean(m_model_csi, axis=0)[1])
        event.optional.plot_scalar("Model/Testing/mFAR", np.nanmean(m_model_far, axis=0)[1])
        event.optional.plot_scalar("Model/Testing/mPOD", np.nanmean(m_model_pod, axis=0)[1])
    ...
    event.optional.plot_scalar("Persistence/Testing/mCSI", np.nanmean(m_per_csi, axis=0)[1])
    event.optional.plot_scalar("Persistence/Testing/mFAR", np.nanmean(m_per_far, axis=0)[1])
    event.optional.plot_scalar("Persistence/Testing/mPOD", np.nanmean(m_per_pod, axis=0)[1])
