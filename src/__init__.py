...
__description__   = "End-to-End Prediction of Lightning Events from Geostationary Satellite Images."
...
def setup_logging(argv):
    ...
    log_pt = log_pt / f"{datetime.datetime.now().strftime('%Y%m%d-%H%M%S%f')}_{socket.gethostname()}{('_' + log_tag) if log_tag else ''}.log"
