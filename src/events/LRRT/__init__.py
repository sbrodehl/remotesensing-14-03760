...
def main(state, event):
    ...
    class TrainIter(TrainDataLoaderIter):
        def inputs_labels_from_batch(self, batch_data):
            return batch_data['SOURCE'], torch.squeeze(torch.squeeze(batch_data['TARGET'], -3), -3)

    train_data_iter = TrainIter(train_loader)
    lrf = LRFinder(
        state["model"], state["optimizer"], state["criterion"]
    )
    lrf.range_test(
        train_data_iter, end_lr=state["end_lr"], num_iter=state["num_iter"], smooth_f=0.05, diverge_th=150
    )
    fig, (ax1, ax2) = plt.subplots(ncols=2, sharey='row', figsize=(10, 5), dpi=300)
    fig.suptitle(f"LRRT {optimizer_name} {criterion_name} {model_name}(w{state['binary.window_size']}c{state['channel_bottleneck_depth']}lt{state['LINET.lead_time']}sr{state['search_region_radius_px']})")
    _, suggested_lr = lrf.plot(ax=ax1)
    ax1.set_title("Overview")
    lrf.reset()
    lrf.range_test(
        train_data_iter, start_lr=suggested_lr, end_lr=state["end_lr"], num_iter=state["num_iter"], smooth_f=0., diverge_th=150
    )
    lrf.plot(skip_start=0, skip_end=0, suggest_lr=False, ax=ax2)  # to inspect the loss-learning rate graph
    ax2.set_title("Detail")
    fig.savefig(f"LRRT-{optimizer_name}-{criterion_name}-{model_name}-w{state['binary.window_size']}-c{state['channel_bottleneck_depth']}-lt{state['LINET.lead_time']}-sr{state['search_region_radius_px']}.{state['image_format']}")
    lrf.reset()
