...
def main(state, event):  # noqa: C901 too-complex pylint: disable=too-many-statements
    ...
    with state.temporary({
        ...
        cache_loader = event.dataloader(subset="train", use_cache=False, deterministic=True)
        ds_hash_fp = Path(state["log.dir"]).parent / f"{cache_loader.dataset.get_hash()}.lock"
    ...
    try:
        ...
        ds_loader_hash_state = {k: state[k] if not isinstance(state[k], enum.Enum) else state[k].name for k in sorted(list(
            k for k in state.all.keys()
            if ("data.loader" in k or "dataset.binary" in k) and not (
                "path" in k
                or "shuffle" in k
                or "Class" in k
                or "cache" in k or "thread" in k
                or "loader.iterable" in k or "loader.mappable" in k
        ...
        loader_ds_digest = hashlib.sha512('-'.join([str(ds_loader_hash_state), str(train_loader.dataset.samples)] + [pipe_digs[pipe] for pipe in sorted(pipe_digs)]).encode("utf-8")).hexdigest()
        ...
        if not check_ds_kw_exists(state, ds_hash_fp):
            ...
            with torch.no_grad():
                tqdm_batch = tqdm(desc="Warm-Up Training Batches", **state["tqdm_kwargs"])
            ...
            with torch.no_grad():
                tqdm_batch = tqdm(desc="Warm-Up Validation Batches", **state["tqdm_kwargs"])
    ...
    tqdm_epoch = tqdm(total=state["epochs"], position=1, desc="Epoch", initial=state["start_epoch"], **state["tqdm_kwargs"])
    ...
    for state["current_epoch"] in range(state["start_epoch"], state["epochs"]):
        ...
        for state["current_batch"], state["sample"] in enumerate(train_loader):
            ...
            state["sample"] = {k: event.optional.to_device(v, altfn=lambda a: a) for k, v in state["sample"].items()}
            event.step()
            if event.plot_every() and not state["log.slim"]:
                ...
                probabilities = torch.nn.Softmax(dim=1)(state["logits"].detach().clone())
                threshold = state["EMA_threshold"].mean.item() if "EMA_threshold" in state else 0.5
                predictions = (probabilities >= threshold).to(probabilities.dtype)
                event.optional.plot_imgs("Model/Probabilities", print_helper(probabilities[:, -1:], batched_data_no=state["batched_data_no"]))
                event.optional.plot_imgs("Model/Predictions", print_helper(predictions[:, -1:], batched_data_no=state["batched_data_no"]))
            state["step"] += 1
        ...
        if "num_batches" not in state:
            state["num_batches"] = (state["current_batch"] + 1) if "current_batch" in state else 0
        elif state["current_batch"] + 1 != state["num_batches"]:
            LOGGER.warning(f"Unexpected amount of batches (got {state['current_batch'] + 1}, expected {state['num_batches']}).")
...
def validate(state, event):  # noqa: C901 too-complex pylint: disable=too-many-statements
    ...
    comps = {}
    tqdm_batch = tqdm(total=state["validation_num_batches"], desc="Validation Batches", position=2, **state["tqdm_kwargs"])
    early_break_is_on = "validation_batches" in state and state["validation_batches"] > 0
    with torch.no_grad():
        for batch_ix, sample in enumerate(state["validation_dataloader"]):
            ...
            if "padding_px" in state and state["padding_px"]:
                mask |= torch.zeros_like(out, dtype=torch.int8)[:, -1:].unsqueeze(1).to(torch.bool).clone()
            ...
            lat_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "LAT"][0]
            lon_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "LON"][0]
            year_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "YEAR"][0]
            month_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "MONTH"][0]
            day_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "DAY"][0]
            hour_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "HOUR"][0]
            minute_idx = [idx for idx, (key, enu) in enumerate(state['META_channel_tensor_order']) if key == "MINUTE"][0]
            if state["dump_validation_results"] and state["epochs_seen"] == state["epochs"]:
                for (lat, lon, year, month, day, hour, minute, logits, nn_sk, in_ch) in zip(
                    ...
                    dt_c.append({
                        "LAT": lat,
                        "LON": lon,
                        "NN_PREDICTIONS": (torch.nn.Softmax(dim=1)(logits) >= state["EMA_threshold"].mean.item()).to(torch.int32),
                        "NN_SKILL": nn_sk,
                        "CHANNELS": in_ch,
                    })
                    comps[tag] = dt_c
    ...
    LOGGER.info(f"Avg. Batch CSI: {mcsi.mean[1] if mcsi.mean is not None else 0.0:.5f} +/- {mcsi.std[1] if mcsi.std is not None else 0.0:.5f}")
    LOGGER.info(f"Complete CSI: {model_csi if model_csi is not None else 0.0:.5f}")
    event.optional.plot_scalar("Skill/Validation/CSI", model_csi if model_csi is not None else 0.0)
    event.optional.plot_scalar("Skill/Validation/mCSI", mcsi.mean[1] if mcsi.mean is not None else 0.0)
    event.optional.plot_scalar("Skill/Validation/mFAR", mfar.mean[1] if mfar.mean is not None else 0.0)
    event.optional.plot_scalar("Skill/Validation/mPOD", mpod.mean[1] if mpod.mean is not None else 0.0)
    if state["dump_validation_results"] and state["epochs_seen"] == state["epochs"]:
        for tag, values in comps.items():
            if "composite_blocks" in state and np.array(state["composite_blocks"]).prod() == 1 == len(values):
                continue
            blocks = state["composite_blocks"]
            if len(blocks) != 2:
                raise NotImplementedError
            idx_lut: dict = {tag: idx for idx, tag in enumerate(sorted(next(iter(values)).keys()))}
            values = [(
            ...
            values = [values[s:e] for (s, e) in [(ridx * len(values) // blocks[0], len(values) // blocks[0] + ridx * len(values) // blocks[0]) for ridx in range(blocks[0])]]
            for ridx, row in enumerate(values):
                row.sort(key=lambda largs: largs[1])
                values[ridx] = [i[-1] for i in row]
            values = {tt: torch.stack([torch.stack([e[tt] for e in row], dim=0) for row in values], dim=0) for tt in idx_lut.keys()}
            for tt in values:
                t = values[tt]
                if "padding_px" in state and state["padding_px"]:
            ...
            dd = Path(state['log.dir']) / "validation-results" / f"epoch-{state['current_epoch']}"
...
def dump_model(state):
    model_pt = Path(state["log.dir"]) / f"{state['model'].__class__.__name__}.{state['log.repository_state'][0][-1]}.{str(state['epochs_seen']).zfill(len(str(state['epochs'])))}.torch"
    torch.save(
        {
            f"{state['model'].__class__.__name__}": state["model"].state_dict(),
            "Model_Threshold": state["EMA_threshold"].mean.item() if "EMA_threshold" in state and state["EMA_threshold"] is not None else None,
        }, model_pt)
    LOGGER.debug(f"Saved model state as: {model_pt}")


def to_one_hot_vector(labels, num_classes):
    return torch.squeeze(torch.movedim(
        torch.eye(num_classes, dtype=labels.dtype, device=labels.device)[labels], -1, 1
    ), -3)
